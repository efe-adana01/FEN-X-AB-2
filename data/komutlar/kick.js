...
exports.run = async (client, message, args) => {
  ...
  if (!args[1])
    return message.channel.send(
      `Lütfen bir neden belirtin! Nedeni olmayan birini yasaklayamazsın, değil mi?`
    );
  if (!Reason)
    return message.channel.send(
      `Lütfen bir neden belirtin! Nedeni olmayan birini yasaklayamazsın, değil mi?`
  ...
  if (!User.kickable)
    return message.channel.send(
      `
Bu kullanıcıyı atamazsınız, benden daha yüksek bir role sahip olabilir veya benimle aynı role sahip olabilir
.`
    );
